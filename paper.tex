...
Using the free tier of Mediastack's news API, we are able to search a corpus of historical news articles, getting metadata about the articles and the URL pointing to their content. Critically, the Mediastack API provides exact publication times so each article can properly be associated with a stock price movement event. We can then use the URL to download the content of the article. Extracting main article text from the HTML can be done with the BeautifulSoup python library on a source by source basis. 
